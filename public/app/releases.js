...
export const RELEASES = [
  {
    ...
    "kind": "CAPABILITY",
    "date": "2026-09-08",
    "summary": "The app can be installed as an app again, and it will say whether there is a new version when asked.",
    "added": [
      "A \"Check for a new version\" button, under If something is wrong in the About panel, with the version this copy is running written beside it. It answers either way: that this is the newest version, that a new one is ready and waiting, or that the check could not reach the network and so learned nothing. Until now the app only spoke when the browser happened to notice, and an installed app that is opened rarely may not notice for a long time."
    ],
    "fixed": [
      "A browser will offer to install this as an app again. Chrome and Edge pick the largest icon an app offers when deciding whether it can be installed. The one they picked here could not be drawn at all, so both concluded the app had no usable icon and quietly stopped offering — while the other icons beside it were correct the whole time. The drawing is repaired and the offer is back.",
      "The icon on a home screen is no longer cropped into. A phone or tablet crops an app icon to whatever shape it likes. The app now also supplies a version drawn with room around the edges, so nothing is cut off."
    ],
    "broken": [
  ...
  },
  {
    ...
    "kind": "ITERATION",
    "date": "2026-09-04",
    "summary": "The greyer text is a little lighter, in both themes.",
    "added": [],
    "fixed": [
      "The dimmed text — hints, counts, the line under a card — is easier to read. It was already above the floor and is further above it now, in the light theme and the dark one.",
      "A colour fix made in the last few days would never have reached you. The app only fetches a new copy of itself when its version changes, and that change had not bumped one — so an installed app would have kept serving the old colours indefinitely. Now it carries a version, so it arrives."
    ],
    "broken": [
      "Two devices still cannot both write. Importing replaces everything, so passing one file back and forth loses whatever the other device added.",
      "A card still does not say how long something has been waiting, what it is worth, or whether there is filament for it."
